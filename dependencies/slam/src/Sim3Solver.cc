...
cv::Mat Sim3Solver::iterate(int nIterations, bool &bNoMore, vector<bool> &vbInliers, int &nInliers)
{
    ...
    cv::Mat P3Dc1i(3,3,CV_64F);
    cv::Mat P3Dc2i(3,3,CV_64F);
...
void Sim3Solver::Project(const vector<cv::Mat> &vP3Dw, vector<cv::Mat> &vP2D, cv::Mat Tcw, cv::Mat K)
{
    ...
    for(size_t i=0, iend=vP3Dw.size(); i<iend; i++)
    {
        ...
        double invz = 1/(P3Dc.at<double>(2));
        double x = P3Dc.at<double>(0)*invz;
        double y = P3Dc.at<double>(1)*invz;

        vP2D.push_back((cv::Mat_<double>(2,1) << fx*x+cx, fy*y+cy));
    }
}
...
void Sim3Solver::FromCameraToImage(const vector<cv::Mat> &vP3Dc, vector<cv::Mat> &vP2D, cv::Mat K)
{
    ...
    {
        double invz = 1/(vP3Dc[i].at<double>(2));
        double x = vP3Dc[i].at<double>(0)*invz;
        double y = vP3Dc[i].at<double>(1)*invz;
