...
namespace ORB_SLAM {
	...
	void LocalMapping::MapPointCulling() {
		...
		// Debug
		/*int erasedBecauseBad = 0;
		int erasedBecauseRatio = 0;
		int erasedBecauseObs = 0;
		int erasedBecauseKFid = 0;*/
		...
		/*cout << "MapPointCulling ";
		cout << "erasedBecauseBad: " << erasedBecauseBad << " - ";
		cout << "erasedBecauseRatio: " << erasedBecauseRatio << " - ";
		cout << "erasedBecauseObs: " << erasedBecauseObs << " - ";
		cout << "erasedBecauseKFid: " << erasedBecauseKFid << " - ";
		cout << endl;*/
	}
	...
	void LocalMapping::CreateNewMapPoints() {
		...
		const double fx1 = mpCurrentKeyFrame->fx;
		const double fy1 = mpCurrentKeyFrame->fy;
		const double cx1 = mpCurrentKeyFrame->cx;
		const double cy1 = mpCurrentKeyFrame->cy;
		const double invfx1 = 1.0f / fx1;
		const double invfy1 = 1.0f / fy1;

		const double ratioFactor = 1.5f * mpCurrentKeyFrame->GetScaleFactor();
		...
		for (size_t i = 0; i < vpNeighKFs.size(); i++) {
			...
			const double invfx2 = 1.0f / fx2;
			const double invfy2 = 1.0f / fy2;
			...
			for (size_t ikp = 0, iendkp = vMatchedKeysUn1.size(); ikp < iendkp; ikp++) {
				...
				cv::Mat xn1 = (cv::Mat_<double>(3, 1) << (kp1.pt.x - cx1) * invfx1, (kp1.pt.y - cy1) * invfy1, 1.0);
				cv::Mat ray1 = Rwc1*xn1;
				cv::Mat xn2 = (cv::Mat_<double>(3, 1) << (kp2.pt.x - cx2) * invfx2, (kp2.pt.y - cy2) * invfy2, 1.0);
				...
				double sigmaSquare1 = mpCurrentKeyFrame->GetSigma2(kp1.octave);
				double x1 = Rcw1.row(0).dot(x3Dt) + tcw1.at<double>(0);
				double y1 = Rcw1.row(1).dot(x3Dt) + tcw1.at<double>(1);
				double invz1 = 1.0 / z1;
				...
				double x2 = Rcw2.row(0).dot(x3Dt) + tcw2.at<double>(0);
				double y2 = Rcw2.row(1).dot(x3Dt) + tcw2.at<double>(1);
				double invz2 = 1.0 / z2;
				...
				double ratioDist = dist1 / dist2;
				double ratioOctave = mpCurrentKeyFrame->GetScaleFactor(kp1.octave) / pKF2->GetScaleFactor(kp2.octave);
