...
void Optimizer::BundleAdjustment(const vector<KeyFrame *> &vpKFs, const vector<MapPoint *> &vpMP, int nIterations, bool* pbStopFlag)
{
    ...
    for(size_t i=0, iend=vpMP.size(); i<iend;i++)
    {
        ...
        for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            double invSigma2 = pKF->GetInvSigma2(kpUn.octave);
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    const double chi2[4]={9.210,7.378,5.991,5.991};
    ...
    //cout << "Optimizer: " << nInitialCorrespondences << " - Bad: " << nBad << " - Null: " << nullKeyPointsCount << endl;
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad())
            {
                ...
                e->setMeasurement(obs);
                double sigma2 = pKFi->GetSigma2(kpUn.octave);
                double invSigma2 = pKFi->GetInvSigma2(kpUn.octave);
                e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
...
int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches1, g2o::Sim3 &g2oS12, double th2)
{
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolverX::LinearSolverType * linearSolver;
    ...
    vector<double> vSigmas12, vSigmas21;
    vector<size_t> vnIndexEdge;
