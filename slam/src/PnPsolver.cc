    ...
    for(size_t i=0, iend=vpMapPointMatches.size(); i<iend; i++)
    {
        ...
        if(pMP)
        {
            if(!pMP->isBad())
            {
                ...
                mvP3Dw.push_back(cv::Point3d(Pos.at<double>(0),Pos.at<double>(1), Pos.at<double>(2)));
    ...
    while(mnIterations<mRansacMaxIts || nCurrentIterations<nIterations)
    {
        ...
        if(mnInliersi>=mRansacMinInliers)
        {
            ...
            if(mnInliersi>mnBestInliers)
            {
                ...
                mBestTcw = cv::Mat::eye(4,4,CV_64F);
...
bool PnPsolver::Refine()
{
    ...
    if(mnInliersi>mRansacMinInliers)
    {
        ...
        Rcw.convertTo(Rcw,CV_64F);
        tcw.convertTo(tcw,CV_64F);
        mRefinedTcw = cv::Mat::eye(4,4,CV_64F);
...
void PnPsolver::CheckInliers()
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        double Xc = mRi[0][0]*P3Dw.x+mRi[0][1]*P3Dw.y+mRi[0][2]*P3Dw.z+mti[0];
        double Yc = mRi[1][0]*P3Dw.x+mRi[1][1]*P3Dw.y+mRi[1][2]*P3Dw.z+mti[1];
        double invZc = 1/(mRi[2][0]*P3Dw.x+mRi[2][1]*P3Dw.y+mRi[2][2]*P3Dw.z+mti[2]);
